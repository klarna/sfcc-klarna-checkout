 ...
 */
function createKlarnaOrder(cart, localeObject, req) {
 ...
 */
function updateKlarnaOrder(cart, localeObject, klarnaOrderID) {
    var klarnaHttpService = new KlarnaHttpService();
    var klarnaApiContext = new KlarnaApiContext();
    var orderRequestBuilder = new KlarnaOrderRequestBuilder();
    var requestBody = orderRequestBuilder.buildRequest({
...
server.get('Start', server.middleware.https, function (req, res, next) {
    ...
    if (!klarnaOrderID) {
        checkoutSnippet = createKlarnaOrder(cart, localeObject, req);
    } else {
        checkoutSnippet = updateKlarnaOrder(cart, localeObject, klarnaOrderID);
    }
...
server.get('Confirmation', server.middleware.https, function (req, res, next) {
    ...
    var klarnaOrderObject = getKlarnaOrder(klarnaOrderID, localeObject);
    ...
    if (placeOrderResult.error) {
        res.redirect(URLUtils.https('KlarnaCheckout-Start', 'PlaceOrderError', placeOrderResult).toString());
    } else {
...
server.get('UpdateKlarnaCheckout', server.middleware.https, function (req, res, next) {
    ...
    var localeObject = utils.getLocaleObject(null, req);
    ...
    var klarnaOrderID = req.session.privacyCache.get('klarnaOrderID');
    if (!klarnaOrderID) {
        isUpdated = createKlarnaOrder(cart, localeObject, req);
    } else {
        isUpdated = updateKlarnaOrder(cart, localeObject, klarnaOrderID);
    }
...
server.post('Push', server.middleware.https, function (req, res, next) {
    ...
    var localeObject = utils.getLocaleObject(klarnaCountry);
    var klarnaOrderObject = getKlarnaOrder(klarnaOrderID, localeObject, true);
